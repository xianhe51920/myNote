...
| 类型                 | 字节 | 二进制范围                 | 十进制约等于   | 适用情况           |
| -------------------- | ---- | -------------------------- | -------------- | ------------------ |
| `int`                | 4    | \(-2^{31}\) ~ \(2^{31}-1\) | ≈ ±2.147×10⁹   | 单值 ≤ 2×10⁹       |
| `unsigned int`       | 4    | \(0\) ~ \(2^{32}-1\)       | ≈ 0 ~ 4.29×10⁹ | 只用正数且 ≤ 4e9   |
| `long long`          | 8    | \(-2^{63}\) ~ \(2^{63}-1\) | ≈ ±9.22×10¹⁸   | 单值 ≤ 9×10¹⁸      |
| `unsigned long long` | 8    | \(0\) ~ \(2^{64}-1\)       | ≈ 1.84×10¹⁹    | 超大正数（极少用） |
...
| 幂次         | 十进制近似                | 对应类型上限                                  |
| ------------ | ------------------------- | --------------------------------------------- |
| \(10^9\)     | 1,000,000,000             | < int max                                     |
| \(10^{18}\)  | 1,000,000,000,000,000,000 | < long long max                               |
| \(2^{31}\)   | 2,147,483,648             | 比 int max 大 1 → 用 long long                |
| \(2^{31}-1\) | 2,147,483,647             | int 最大值                                    |
| \(2^{63}\)   | 9,223,372,036,854,775,808 | 比 long long max 大 1 → 用 unsigned long long |
| \(2^{63}-1\) | 9,223,372,036,854,775,807 | long long 最大值                              |

---

### ⚠️ 避坑规则（必看）

1. **单值**：看到 \(10^9\) → `int`；看到 \(10^{18}\) → `long long`。
2. **求和**：若数组长度 × 单值上限 > 当前类型范围，总和变量要升一级。
3. **乘法中间值**：`int a=1e6, b=1e6; long long c = 1LL * a * b;` —— 乘 `1LL` 强制提升。
4. **不确定时**：一律用 `long long`（除非二维数组过大，内存不够才精细算）。
5. **不要用 `long`**：不同平台长度不一（Windows 4 字节，Linux 8 字节），刷题统一用 `int` 和 `long long`。